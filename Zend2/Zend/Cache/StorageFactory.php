...
abstract class StorageFactory
{
     ...
     */
    private static $aryAdapterInstances = array();  // to guarantee singleton per config
    public static function factory($cfg)
    {
        ...
        $cfgAsKey = $cfg['adapter']['options']['server']['host'] . ':' . $cfg['adapter']['options']['server']['port'];
        if (array_key_exists($cfgAsKey, self::$aryAdapterInstances)) {
            return self::$aryAdapterInstances[$cfgAsKey];
        }
        ...
        self::$aryAdapterInstances[$cfgAsKey] = $adapter;
